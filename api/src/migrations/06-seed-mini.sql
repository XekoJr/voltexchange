...
--
-- Requer: pgcrypto (para hashing bcrypt compatível com a API)
-- Todos os utilizadores têm password: senha123
--
-- Estrutura:
--   10 utilizadores | 10 contadores | 50 leituras | 20 ofertas | 10 ordens
-- =============================================================================
...
-- =============================================================================
-- UTILIZADORES (10)
-- Distribuição de saldos pensada para testar casos limite:
--   - Alice, David, Gabriela: compradores com saldo suficiente
--   - Fernando: saldo zero → testa erro "saldo insuficiente"
--   - João, Hugo: vendedores principais
-- =============================================================================
...
-- utilizador_id: Alice=1, Bruno=2, Carla=3, David=4, Eva=5,
--               Fernando=6, Gabriela=7, Hugo=8, Inês=9, João=10

-- =============================================================================
-- CONTADORES (10)
-- Distribuídos pelas regiões Norte, Centro, Sul
-- Alguns contadores ficarão em MANUTENCAO após as leituras anómalas
-- =============================================================================
INSERT INTO Contadores (utilizador_id, numero_serie, estado, regiao) VALUES
    (1,  'CTR-NORTE-001', 'ATIVO',     'Norte'),   -- contador_id=1  (Alice)
    (1,  'CTR-NORTE-002', 'ATIVO',     'Norte'),   -- contador_id=2  (Alice)
    (2,  'CTR-CENTRO-001','ATIVO',     'Centro'),  -- contador_id=3  (Bruno)
    (4,  'CTR-NORTE-003', 'ATIVO',     'Norte'),   -- contador_id=4  (David)
    (5,  'CTR-SUL-001',   'ATIVO',     'Sul'),     -- contador_id=5  (Eva)
    (7,  'CTR-CENTRO-002','ATIVO',     'Centro'),  -- contador_id=6  (Gabriela)
    (7,  'CTR-SUL-002',   'ATIVO',     'Sul'),     -- contador_id=7  (Gabriela)
    (8,  'CTR-NORTE-004', 'ATIVO',     'Norte'),   -- contador_id=8  (Hugo)
    (9,  'CTR-CENTRO-003','ATIVO',     'Centro'),  -- contador_id=9  (Inês)
    (10, 'CTR-SUL-003',   'ATIVO',     'Sul');     -- contador_id=10 (João)

-- =============================================================================
-- LEITURAS (50)
-- 40 normais + 10 anómalas (5 temperatura > 80, 5 com erro_codigo)
-- Datas dentro das partições 2025-01 a 2026-12
-- Nota: o trigger trg_DetectarAnomalias alterará contadores para MANUTENCAO
--       nas 10 leituras anómalas inseridas abaixo
-- =============================================================================

-- --- 40 leituras normais (temperatura 20-60°C, sem erro_codigo) ---
...
-- --- 10 leituras anómalas (activam trg_DetectarAnomalias → contador MANUTENCAO) ---
-- 5 com temperatura > 80°C
INSERT INTO Leituras (contador_id, data_hora, kwh_leitura, dados_audit) VALUES
...
-- =============================================================================
-- OFERTAS DE VENDA (20)
-- Vendedores: Gabriela (id=7), João (id=10), Hugo (id=8), David (id=4)
-- Regiões variadas, preços entre 0.10 e 0.18 €/kWh
-- Nota: triggers trg_AutoMatching_Oferta irão disparar sp_MatchingEngine
--       a cada INSERT → desactivar temporariamente se quiser controlo manual
-- =============================================================================

-- Desactivar o trigger de auto-matching durante o seed para inserir dados limpos
...
-- --- Ofertas de Gabriela (id=7) ---
INSERT INTO OfertasVenda (vendedor_id, quantidade_kwh, preco_unitario, estado, regiao) VALUES
    (7, 100.000, 0.1100, 'ATIVA', 'Norte'),    -- oferta_id=1
    (7,  50.000, 0.1200, 'ATIVA', 'Norte'),    -- oferta_id=2
    (7,  75.000, 0.1150, 'ATIVA', 'Centro'),   -- oferta_id=3
    (7,  30.000, 0.1300, 'ATIVA', 'Sul'),      -- oferta_id=4
    (7,  60.000, 0.1050, 'ATIVA',  NULL);      -- oferta_id=5 (qualquer região)

-- --- Ofertas de João (id=10) ---
INSERT INTO OfertasVenda (vendedor_id, quantidade_kwh, preco_unitario, estado, regiao) VALUES
    (10, 40.000, 0.1250, 'ATIVA', 'Centro'),   -- oferta_id=6
    (10, 80.000, 0.1350, 'ATIVA', 'Norte'),    -- oferta_id=7
    (10, 25.000, 0.1200, 'ATIVA', 'Sul'),      -- oferta_id=8
    (10, 55.000, 0.1400, 'ATIVA', NULL),       -- oferta_id=9
    (10, 20.000, 0.1100, 'ATIVA', 'Norte');    -- oferta_id=10

-- --- Ofertas de Hugo (id=8) ---
INSERT INTO OfertasVenda (vendedor_id, quantidade_kwh, preco_unitario, estado, regiao) VALUES
    (8,  35.000, 0.1300, 'ATIVA', 'Norte'),    -- oferta_id=11
    (8,  45.000, 0.1450, 'ATIVA', 'Centro'),   -- oferta_id=12
    (8,  15.000, 0.1600, 'ATIVA', 'Sul'),      -- oferta_id=13
    (8,  90.000, 0.1550, 'ATIVA', NULL);       -- oferta_id=14

-- --- Ofertas de David (id=4) ---
INSERT INTO OfertasVenda (vendedor_id, quantidade_kwh, preco_unitario, estado, regiao) VALUES
    (4,  25.000, 0.1700, 'ATIVA', 'Norte'),    -- oferta_id=15
    (4,  50.000, 0.1800, 'ATIVA', 'Centro'),   -- oferta_id=16
    (4,  30.000, 0.1650, 'ATIVA', 'Sul'),      -- oferta_id=17
    (4,  20.000, 0.1750, 'ATIVA', NULL),       -- oferta_id=18
    (4,  65.000, 0.1600, 'ATIVA', 'Norte'),    -- oferta_id=19
    (4,  10.000, 0.1550, 'ATIVA', 'Centro');   -- oferta_id=20

-- =============================================================================
-- ORDENS DE COMPRA (10)
-- Compradores: Alice (id=1), Bruno (id=2), Carla (id=3), David (id=4), Eva (id=5)
--
-- Pares compatíveis com as ofertas acima (para testar matching):
--   Ordem 1 (Alice, Norte, max=0.13)  ↔ Ofertas 1, 2, 10 (Gabriela/João, Norte, ≤0.13)
--   Ordem 2 (Bruno, Centro, max=0.14) ↔ Oferta 3 (Gabriela, Centro, 0.115) + Oferta 6 (João, Centro, 0.125)
--   Ordem 3 (Carla, Sul, max=0.15)    ↔ Ofertas 4, 8 (Gabriela/João, Sul, ≤0.15)
--   Ordem 4 (David, NULL, max=0.12)   ↔ Oferta 5 (Gabriela, NULL, 0.105) + Oferta 10 (João, Norte, 0.11)
--   Ordem 9 (Fernando, Norte, max=0.15) → FALHA saldo insuficiente (saldo=0)
-- =============================================================================
INSERT INTO OrdensCompra (comprador_id, quantidade_kwh, preco_maximo, estado, regiao) VALUES
    (1,  20.000, 0.1300, 'PENDENTE', 'Norte'),   -- ordem_id=1  Alice ← compatível
    (2,  15.000, 0.1400, 'PENDENTE', 'Centro'),  -- ordem_id=2  Bruno ← compatível
    (3,  10.000, 0.1500, 'PENDENTE', 'Sul'),     -- ordem_id=3  Carla ← compatível
    (4,  25.000, 0.1200, 'PENDENTE',  NULL),     -- ordem_id=4  David (compra qualquer região)
    (5,   5.000, 0.1400, 'PENDENTE', 'Norte'),   -- ordem_id=5  Eva
    (1,  50.000, 0.1600, 'PENDENTE', 'Centro'),  -- ordem_id=6  Alice (segundo pedido)
    (2,  30.000, 0.1700, 'PENDENTE',  NULL),     -- ordem_id=7  Bruno (qualquer região)
    (3,  40.000, 0.1800, 'PENDENTE', 'Norte'),   -- ordem_id=8  Carla
    (6,  10.000, 0.1500, 'PENDENTE', 'Norte'),   -- ordem_id=9  Fernando ← SALDO ZERO (teste falha)
    (9,   8.000, 0.1300, 'PENDENTE', 'Sul');     -- ordem_id=10 Inês
